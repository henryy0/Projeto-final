            ...
            <?php
                ...
                for($cb=1;$cb<=$boxDisplay;$cb++){
                    if(($cb >= $currentMonthFirstDay || $currentMonthFirstDay == 1) && $cb <= ($totalDaysOfMonthDisplay)){
                        ...
                        global $db;
                        $result = $db->query("SELECT titulo FROM Calendario WHERE Data = '".$currentDate."' AND status_calendario = 1");
...
function getEvents($date = ''){
    ...
    global $db;
    $result = $db->query("SELECT titulo FROM Calendario WHERE Data = '".$date."' AND status_calendario = 1");
    if($result->num_rows > 0){
        ...
        while($row = $result->fetch_assoc()){ $i++;
            $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">'.$i.'.</span>'.$row['titulo'].'</li>';
        }
        $eventListHTML .= '</ul>';
    }
    echo $eventListHTML;
